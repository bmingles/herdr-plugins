...
Copied from `agent-caffeinate/src/sock.py` (see that file on why plugins duplicate
rather than share) and extended with `snapshot()` and `pane_read()`.
...
def request(socket_path, method, params=None, timeout=5.0):
    ...
    try:
        ...
        payload = json.dumps({"id": "workspace-time-tracker",
                              "method": method,
                              "params": params or {}})
        try:
...
def snapshot(socket_path, timeout=5.0):
    """The whole session snapshot: focus, panes, workspaces. ~0.35 ms."""
    ...
    try:
        ...
        raise ProtocolError("session.snapshot reply has no result.snapshot")


def pane_read(socket_path, pane_id, source="visible", timeout=5.0):
    """The text currently on a pane's screen.

    `source` is **required** by the server; valid values are `visible`, `recent`,
    `recent_unwrapped` and `detection` (not `screen`/`scrollback`). The text lives at
    `result.read.text`, not `result.text`. Measured at 0.59 ms.

    Returns `None` when the pane is gone, which is an ordinary race rather than an error.
    """
    reply = request(socket_path, "pane.read",
                    {"pane_id": pane_id, "source": source}, timeout=timeout)
    if "error" in reply:
        code = (reply["error"] or {}).get("code")
        if code in ("pane_not_found", "not_found"):
            return None
        raise ProtocolError("pane.read failed: %s" % json.dumps(reply["error"]))
    try:
        return reply["result"]["read"]["text"]
    except (KeyError, TypeError):
        raise ProtocolError("pane.read reply has no result.read.text")
