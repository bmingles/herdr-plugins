...
Copied verbatim from `agent-caffeinate/src/daemonize.py`. The duplication is deliberate:
`herdr plugin install <owner>/<repo>/<subdir>` fetches a single subdirectory, so a plugin
cannot import across sibling directories. Keep the copies in sync by hand.
...
#
# `track report` is a command you run every day, but a GitHub-installed plugin root is
# `~/.config/herdr/plugins/github/<id>-<hash>/<subdir>` and a linked one is wherever the
# user's checkout lives, so nothing outside the plugin can name `bin/track` without first
# digging the path out of `herdr plugin list --json`. The state directory *is* fixed at
# `~/.local/state/herdr/plugins/workspace-time-tracker`, so the daemon keeps an executable
# shim there for the README to point a `PATH` symlink at. It is refreshed on every daemon
# start, so it follows the plugin when a reinstall moves its root.

LAUNCHER_HEADER = (
    "# Generated by workspace-time-tracker. Do not edit -- rewritten on every daemon\n"
    "# start, so it follows the plugin when a reinstall moves its directory.\n"
)
...
def write_launcher(path, entrypoint):
    ...
    runs this on every `workspace.focused`. Never raises: a convenience shim must not be
